...
class InventoryResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make('Інформація про інвентар')
                    ->schema([
                        Forms\Components\Grid::make(2)
                            ->schema([
                                Forms\Components\TextInput::make('name')
                                    ->label('Назва')
                                    ->required()
                                    ->placeholder('напр. Викрутка крестова'),
                Forms\Components\Select::make('part_type_id')
                    ->label('Тип інвентарю')
                                    ->options(function () {
                                        return \App\Models\PartType::where('name', 'like', '%Інвентар%')
                                            ->orderBy('name', 'asc')
                                            ->get()
                                            ->mapWithKeys(function ($partType) {
                                                // Прибираємо слово "Інвентар" з назви для відображення
                                                $displayName = str_replace('Інвентар', '', $partType->name);
                                                $displayName = str_replace('інвентар', '', $displayName);
                    ...
                    ->required()
                                    ...
                                    ->formatStateUsing(function ($state) {
                                        ...
                                        // Прибираємо слово "Інвентар" з назви для відображення вибраного значення
                                        $displayName = str_replace('Інвентар', '', $partType->name);
                                        $displayName = str_replace('інвентар', '', $displayName);
                                        return trim($displayName);
                                    })
                    ...
                    ->createOptionForm([
                                    ...
                                    ->createOptionUsing(function (array $data, Forms\Get $get, Forms\Set $set): int {
                                        // Додаємо "Інвентар" до назви при створенні, якщо його немає
                                        $name = $data['name'];
                                        if (stripos($name, 'Інвентар') === false && stripos($name, 'інвентар') === false) {
                                            $name = 'Інвентар ' . $name;
                                        }
            ...
            ->columns([
                Tables\Columns\TextColumn::make('partType.name')
                    ...
                    ->formatStateUsing(function (Part $record) {
                        ...
                        // Прибираємо слово "Інвентар" з назви типу
                        $typeLabel = str_replace('Інвентар', '', $typeLabel);
                        $typeLabel = str_replace('інвентар', '', $typeLabel);
                        ...
                        Forms\Components\Grid::make(2)
                            ->schema([
                                Forms\Components\TextInput::make('quantity_to_add')
                                    ...
                                    ->default(1)
                                    ->minValue(1)
                                    ->helperText('Введіть кількість інвентарю, який надходить на склад')
                                    ->live(),
                                Forms\Components\TextInput::make('cost_per_unit')
                                    ...
                                    ->minValue(0)
                                    ->prefix('₴')
                                    ->helperText('Введіть ціну за одиницю нового інвентарю')
                                    ->live(),
                            ]),
